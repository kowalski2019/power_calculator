...
static String[] subsleft(String left){
    ...
    for(int i = left.length() - 1; i >= 0; i--){
      ...
      if(isOp(left.charAt(i))){
        ...
        // isIdent not correctly does
        // check firstable if we habe any character and then check vor Ident.
  ...
  static String parseCalc(String expr,char sign,int i){
    ...
    a=i; z=i+1;
       ...
       if(isIdent(left)){
        ...
        } else error("Unknown variable left '"+left+"' \n");
       }

       if(isIdent(right)){
         if(isVarAlreadyDefined(right)){
          right=getVarValFromSymTable(right);
         }else error("Unknown variable right '"+right+"' \n");;
       }
